	...
	return ChainingProxy(chain, elementPromise, {
		get (chain, elementPromise, property, receiver) {
			...
			if (promiseInstanceKeys.includes(property)) {
				...
				return new Proxy(Reflect.get(elementPromise, property, receiver), {
			...
			if (methodsReturningNewFrame.includes(property)) {
				...
				// TODO make chainable
				return () => elementPromise.then(element => Reflect.apply(Reflect.get(element, property, receiver), element, [])).then(frame => FrameProxy(chain, frame))
			}
